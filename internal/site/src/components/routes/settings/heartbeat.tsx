		...
		<div>
			<div>
				...
				<p className="text-sm text-muted-foreground leading-relaxed">
					<Trans>
						Send periodic outbound pings to an external monitoring service so you can monitor Anarchy Pulse without exposing it
						to the internet.
					</Trans>
				</p>
			</div>
...
function NotEnabledState({ isLoading }: { isLoading?: boolean }) {
	return (
		<div className={cn("grid gap-4", isLoading && "animate-pulse")}>
			<div>
				<p className="text-sm text-muted-foreground leading-relaxed mb-3">
					<Trans>Set the following environment variables on your Anarchy Pulse hub to enable heartbeat monitoring:</Trans>
				</p>
			...
			<p className="text-sm text-muted-foreground leading-relaxed">
				<Trans>After setting the environment variables, restart your Anarchy Pulse hub for changes to take effect.</Trans>
			</p>
		</div>
	)
}
