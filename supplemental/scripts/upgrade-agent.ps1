...
# Function to upgrade anarchy-pulse-agent with Scoop
function Upgrade-Anarchy PulseAgentWithScoop {
    Write-Host "Upgrading anarchy-pulse-agent with Scoop..."
    scoop update anarchy-pulse-agent
    
    if (-not (Test-CommandExists "anarchy-pulse-agent")) {
        throw "Failed to upgrade anarchy-pulse-agent with Scoop"
    }
    
    return $(Join-Path -Path $(scoop prefix anarchy-pulse-agent) -ChildPath "anarchy-pulse-agent.exe")
}

# Function to upgrade anarchy-pulse-agent with WinGet
function Upgrade-Anarchy PulseAgentWithWinGet {
    Write-Host "Upgrading anarchy-pulse-agent with WinGet..."
    ...
    & winget upgrade --exact --id henrygd.anarchy-pulse-agent --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    if (-not $agentPath) {
        ...
        $agentPath = Find-Anarchy PulseAgent
        if (-not $agentPath) {
            throw "Could not find anarchy-pulse-agent executable path after upgrade"
        }
    }
...
function Get-ServiceConfiguration {
    ...
    if (-not $existingService) {
        throw "anarchy-pulse-agent service does not exist. Please run the installation script first."
    }
    ...
    try {
        ...
        $envVars = & $nssmCommand get anarchy-pulse-agent AppEnvironmentExtra
...
function Update-ServicePath {
    ...
    Write-Host "Updating anarchy-pulse-agent service path..."
    ...
    & $nssmCommand set anarchy-pulse-agent Application $NewAgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to update anarchy-pulse-agent service path"
    }
    
    Write-Host "Service path updated to: $NewAgentPath"
    
    # Start the service
    Start-Anarchy PulseAgentService -NSSMPath $nssmCommand
}
...
try {
    Write-Host "Anarchy Pulse Agent Upgrade Script" -ForegroundColor Cyan
    Write-Host "===========================" -ForegroundColor Cyan
    
    # First: Check if service exists (doesn't require admin)
    $existingService = Get-Service -Name "anarchy-pulse-agent" -ErrorAction SilentlyContinue
    if (-not $existingService) {
        Write-Host "ERROR: anarchy-pulse-agent service does not exist." -ForegroundColor Red
    ...
    Write-Host "Stopping anarchy-pulse-agent service..."
    & $nssmPath stop anarchy-pulse-agent
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Upgrading anarchy-pulse-agent..."
    $newAgentPath = $null
    
    if (Test-CommandExists "scoop") {
        Write-Host "Using Scoop for upgrade..."
        $newAgentPath = Upgrade-Anarchy PulseAgentWithScoop
    }
    elseif (Test-CommandExists "winget") {
        Write-Host "Using WinGet for upgrade..."
        $newAgentPath = Upgrade-Anarchy PulseAgentWithWinGet
    }
    else {
    ...
    if (-not $newAgentPath) {
        $newAgentPath = Find-Anarchy PulseAgent
        if (-not $newAgentPath) {
            throw "Could not find anarchy-pulse-agent executable after upgrade."
        }
    }
    ...
    if ($isAdmin -or $Elevated) {
        ...
        Write-Host ""
        Write-Host "Upgrade completed successfully!" -ForegroundColor Green
        Write-Host "The anarchy-pulse-agent service has been updated to use the new executable path." -ForegroundColor Green
